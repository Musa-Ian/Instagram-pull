...
import { Download, Copy, MoreHorizontal } from "lucide-react"
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      {validMedia.map((item, index) => (
        <Card key={item.url || index} className="overflow-hidden group relative aspect-square">
          {item.type === "image" ? (
            <Image
              src={item.url}
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="ghost"
                  size="icon"
                  className="bg-black/40 text-white rounded-full h-8 w-8 hover:bg-black/60 backdrop-blur-md"
                >
